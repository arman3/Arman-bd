...
""")
	...
	if api =="":
		mengetik('harap masukan dengan benar')
		exit()
...
def banner():
 ...
 {k} *   * {h}/+/++//
   \ /  |/__//
 [{m}X{h}]{h}v[{m}X{h}]{p}|{b}MBF{p}|==========.
   {h}({m}'{h})  {h}/'|'\           {p}\\
       {h}/  \  \          {p}'
       {h}\_  \_ \_   {p}___{m}MBF {k}2.0{p}___
''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL...JALANKAN ULANG SC!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL....TUMBAL LU KENA CP!!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	print('')
	print(f'''
 {p}* MULTI BRUTEFORCE FACEBOOK
 {b}* PIRMANSX
 {u}* https://github.com/arman3
 * https://facebook.com/groups/316534524011261
 * https://pirmansx.waper.com{k}
.======================.
|  {h}AMBIL ID DARI.....{k}  |
|======================'
| {u}01{k} | DAFTAR TEMAN
| {u}02{k} | DAFTAR PENGIKUT
| {u}03{k} | HASIL CRACK
| {u}00{k} | KELUAR
'======================'
''')
	...
	if _____alvino__adijaya_____ in ['1','01']:
		...
		back()
def error():
	print(f'fitur masih dalam tahap pengembangan')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print('[1] Arman CP king ')
	print('[2] Arman OK king ')
	print('[3] ARMAN	')
	kz = input('\n[!] Pilih : ')
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
		except FileNotFoundError:
			print('Filemu Tidak di temukan')
			time.sleep(3)
			back()
		if len(vin)==0:
			print('ARMAN.C.E.O KING  CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[!] Mau Berapa ID MAX 100 ID : '))
	except ValueError:
		print('[×] Armanvai king, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	for userr in uid:
		try:
			...
			print('error silahkan periksa jaringan')
			exit()
	try:
		print('')
...
''')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		print (f'\033[1;31mMasukkan Katasandi Tambahan Minimal 6 Karakter\nContoh :{h}ririn1234,agus ismail,dono123')
		pwku=input(f'{p}>> Masukkan Password Tambahan :{h} ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
 ...
 {p}* MULTI BRUTEFORCE FACEBOOK
 {b}* PIRMANSX
 {u}* https://github.com/pirmansx
 * https://facebook.com/groups/164201767529837
 * https://pirmansx.waper.com{k}
''')
	print(f'\n#-----------[ Sedang Crack ]-----------#')
	print('')
	print(f'{h}# Hasil {h}☣️ OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'{k}# Hasil {k}☢️ CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
